...
@interface TDeskMessageCell () <CAAnimationDelegate>
@property(nonatomic, strong) TDeskMessageCellData *messageData;

@end

@implementation TDeskMessageCell
...
- (void)setupSubViews {
    ...
    _messageModifyRepliesButton = [[TDeskFitButton alloc] initWithFrame:CGRectMake(0, 0, 12, 12)];
...
- (void)fillWithData:(TDeskMessageCellData *)data {
    ...
    if (data.status == Msg_Status_Fail) {
        ...
        if (data.status == Msg_Status_Sending_2) {
            ...
            if (self.messageData.showReadReceipt && self.messageData.direction == MsgDirectionOutgoing && self.messageData.innerMessage.needReadReceipt &&
                ...
                ![self.messageData isKindOfClass:TDeskSystemMessageCellData.class]) {
    ...
    _timeLabel.text = [TDeskTool convertDateToStr:data.innerMessage.timestamp];
...
- (void)loadAvatar:(TDeskMessageCellData *)data {
...
@implementation TDeskMessageCell (TUILayoutConfiguration)
