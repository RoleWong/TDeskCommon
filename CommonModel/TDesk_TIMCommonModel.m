...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskCommonCellData
...
@interface TDeskCommonTableViewCell () <UIGestureRecognizerDelegate>
@property TDeskCommonCellData *data;
@property UITapGestureRecognizer *tapRecognizer;
@end

@implementation TDeskCommonTableViewCell
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskCommonTextCellData
- (instancetype)init {
    self = [super init];
...
@interface TDeskCommonTextCell ()
@property TDeskCommonTextCellData *textData;
@end

@implementation TDeskCommonTextCell
...
- (void)fillWithData:(TDeskCommonTextCellData *)textData {
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskCommonSwitchCellData
- (instancetype)init {
    self = [super init];
    _margin = 20;
    return self;
...
@interface TDeskCommonSwitchCell ()

@property TDeskCommonSwitchCellData *switchData;
@property(nonatomic, strong) UIView *leftSeparatorLine;

@end

@implementation TDeskCommonSwitchCell
...
- (void)fillWithData:(TDeskCommonSwitchCellData *)switchData {
...
- (void)accept {
    [[V2TIMManager sharedInstance] acceptGroupApplication:_pendencyItem
        ...
        fail:^(int code, NSString *msg) {
          [TDeskTool makeToastError:code msg:msg];
        }];
    self.isAccepted = YES;
}
- (void)reject {
...
@implementation TDeskGroupPendencyCell
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskButtonCellData
...
@implementation TDeskButtonCell {
    UIView *_line;
}
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskFaceCellData
@end

@implementation TDeskFaceCell
- (id)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        [self setupViews];
...
- (void)setData:(TDeskFaceCellData *)data {
    if (!kTUIFaceCellAllowDynamicImageShow) {
        ...
        _face.image = [[TDeskImageCache sharedInstance] getFaceFromCache:data.path];
    }
    [self defaultLayout];
}
...
- (NSDictionary *)facesMap {
    if (!_facesMap || (_facesMap.count != _faces.count )) {
        ...
        if (_faces.count > 0) {
            for (TDeskFaceCellData *data in _faces) {
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskUnReadView
- (id)init {
    self = [super init];
    if (self) {
        [self setupViews];
...
@implementation TDeskConversationPin
+ (instancetype)sharedInstance {
    static TDeskConversationPin *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
      instance = [TDeskConversationPin new];
    });
    return instance;
}
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskCommonContactSelectCellData
...
- (NSComparisonResult)compare:(TDeskCommonContactSelectCellData *)data {
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskCommonContactListPickerCell
...
- (void)layoutSubviews {
    ...
    if ([TDeskConfig defaultConfig].avatarType == TAvatarTypeRounded) {
        ...
        _avatar.layer.masksToBounds = YES;
        _avatar.layer.cornerRadius = [TDeskConfig defaultConfig].avatarCornerRadius;
    }
}
...
//
/////////////////////////////////////////////////////////////////////////////////
@interface TDeskContactListPicker () <UICollectionViewDelegate, UICollectionViewDataSource>
@property(nonatomic) UICollectionView *collectionView;
@property(nonatomic) UIButton *accessoryBtn;
@end

@implementation TDeskContactListPicker
...
- (void)initControl {
    ...
    [self.collectionView registerClass:[TDeskCommonContactListPickerCell class] forCellWithReuseIdentifier:@"PickerIdentifier"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TDeskCommonContactListPickerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PickerIdentifier" forIndexPath:indexPath];

    TDeskCommonContactSelectCellData *data = self.selectArray[indexPath.row];
    if (data.avatarUrl) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TDeskCommonContactSelectCellData *data = self.selectArray[indexPath.item];
    if (self.onCancel) {
        self.onCancel(data);
    }
}
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskProfileCardCellData
...
@implementation TDeskProfileCardCell
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
        [self setupViews];
...
- (void)setupViews {
    ...
    if ([TDeskConfig defaultConfig].avatarType == TAvatarTypeRounded) {
        ...
        self.avatar.layer.masksToBounds = YES;
        self.avatar.layer.cornerRadius = [TDeskConfig defaultConfig].avatarCornerRadius;
    }
    [self.contentView addSubview:_avatar];
...
@interface TDeskAvatarViewController () <UIScrollViewDelegate>
@property UIImageView *avatarView;

@property TDeskScrollView *avatarScrollView;
...
@implementation TDeskAvatarViewController
...
- (void)viewDidLoad {
    ...
    self.avatarScrollView = [[TDeskScrollView alloc] initWithFrame:CGRectZero];
    ...
    TDeskProfileCardCellData *data = self.avatarData;
    /*
     @weakify(self);
...
@implementation TDeskSelectAvatarCardItem
...
@property(nonatomic, strong) TDeskSelectAvatarCardItem *cardItem;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.imageView.layer.cornerRadius = [TDeskConfig defaultConfig].avatarCornerRadius;
...
- (void)updateNormalGroupGridAvatar {
    if (TDeskConfig.defaultConfig.enableGroupGridAvatar && self.cardItem.cacheGroupGridAvatarImage) {
...
- (void)loadData {
    if (self.selectAvatarType == TUISelectAvatarTypeUserAvatar) {
        for (int i = 0; i < UserAvatarCount; i++) {
            TDeskSelectAvatarCardItem *cardItem = [self creatCardItemByURL:UserAvatarURL(i + 1)];
            [self.dataArr addObject:cardItem];
        }
    } else if (self.selectAvatarType == TUISelectAvatarTypeGroupAvatar) {
        if (TDeskConfig.defaultConfig.enableGroupGridAvatar && self.cacheGroupGridAvatarImage) {
            TDeskSelectAvatarCardItem *cardItem = [self creatGroupGridAvatarCardItem];
            [self.dataArr addObject:cardItem];
        }

        for (int i = 0; i < GroupAvatarCount; i++) {
            TDeskSelectAvatarCardItem *cardItem = [self creatCardItemByURL:GroupAvatarURL(i + 1)];
            [self.dataArr addObject:cardItem];
        }
    } else if (self.selectAvatarType == TUISelectAvatarTypeConversationBackGroundCover) {
        TDeskSelectAvatarCardItem *cardItem = [self creatCleanCardItem];
        [self.dataArr addObject:cardItem];
        for (int i = 0; i < BackGroundCoverCount; i++) {
            TDeskSelectAvatarCardItem *cardItem = [self creatCardItemByURL:BackGroundCoverURL(i + 1) fullUrl:BackGroundCoverURL_full(i + 1)];
            [self.dataArr addObject:cardItem];
        }
    }
        ...
        for (int i = 0; i < Community_coverCount; i++) {
            TDeskSelectAvatarCardItem *cardItem = [self creatCardItemByURL:Community_coverURL(i + 1)];
            [self.dataArr addObject:cardItem];
        }
    }
    [self.collectionView reloadData];
...
- (TDeskSelectAvatarCardItem *)creatCardItemByURL:(NSString *)urlStr {
    TDeskSelectAvatarCardItem *cardItem = [[TDeskSelectAvatarCardItem alloc] init];
...
- (TDeskSelectAvatarCardItem *)creatGroupGridAvatarCardItem {
    TDeskSelectAvatarCardItem *cardItem = [[TDeskSelectAvatarCardItem alloc] init];
...
- (TDeskSelectAvatarCardItem *)creatCardItemByURL:(NSString *)urlStr fullUrl:(NSString *)fullUrl {
    TDeskSelectAvatarCardItem *cardItem = [[TDeskSelectAvatarCardItem alloc] init];
    cardItem.posterUrlStr = urlStr;
...
- (TDeskSelectAvatarCardItem *)creatCleanCardItem {
    TDeskSelectAvatarCardItem *cardItem = [[TDeskSelectAvatarCardItem alloc] init];
    cardItem.posterUrlStr = nil;
    cardItem.isSelect = NO;
...
- (void)setCurrentSelectCardItem:(TDeskSelectAvatarCardItem *)currentSelectCardItem {
...
- (void)recoverSelectedStatus {
    NSInteger index = 0;
    for (TDeskSelectAvatarCardItem *card in self.dataArr) {
...
//
/////////////////////////////////////////////////////////////////////////////////
@implementation TDeskCommonAvatarCellData
- (instancetype)init {
    self = [super init];
    if (self) {
        _avatarImage = DefaultAvatarImage;
...
@interface TDeskCommonAvatarCell ()
@property TDeskCommonAvatarCellData *avatarData;
@end

@implementation TDeskCommonAvatarCell
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:reuseIdentifier]) {
        [self setupViews];
        self.selectionStyle = UITableViewCellSelectionStyleNone;
...
@implementation TDeskConversationGroupItem
- (instancetype)init {
    self = [super init];
    if (self) {
        self.unreadCount = 0;
...
@implementation TDeskSendMessageAppendParams
+ (instancetype)defaultConfig {
    TDeskSendMessageAppendParams *params = [[TDeskSendMessageAppendParams alloc] init];
