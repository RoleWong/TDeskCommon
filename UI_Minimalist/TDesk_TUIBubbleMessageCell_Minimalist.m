//
//  TDeskBubbleMessageCell_Minimalist.m
//  TXIMSDK_TUIKit_iOS
//
...
#import "TDesk_TUIBubbleMessageCell_Minimalist.h"
#import <TDeskCommon/TDesk_TIMCommonModel.h>
#import <TDeskCommon/TDesk_TIMDefine.h>

@implementation TDeskBubbleMessageCell_Minimalist
...
- (void)fillWithData:(TDeskBubbleMessageCellData *)data {
...
- (UIImage *)getBubble {
    if (!TDeskIMConfig.defaultConfig.enableMessageBubble) {
        return nil;
    }
...
- (UIImage *)getSameMessageBubble {
    ...
    if (self.bubbleData.direction == MsgDirectionIncoming) {
        UIImage *defaultImage = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"ReceiverTextNodeBkg_Same")];
        ...
        UIImage *defaultImage = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"SenderTextNodeBkg_Same")];
        defaultImage = [defaultImage rtl_imageFlippedForRightToLeftLayoutDirection];
        bubble_SameMsg = [defaultImage resizableImageWithCapInsets:ei resizingMode:UIImageResizingModeStretch];
    }
...
@implementation TDeskBubbleMessageCell_Minimalist (TUILayoutConfiguration)
...
+ (UIImage *)outgoingBubble {
    if (!gOutgoingBubble) {
        UIImage *defaultImage = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"SenderTextNodeBkg")];
        defaultImage = [defaultImage rtl_imageFlippedForRightToLeftLayoutDirection];
        UIEdgeInsets ei = UIEdgeInsetsFromString(@"{12,12,12,12}");
        ei = rtlEdgeInsetsWithInsets(ei);
...
+ (UIImage *)incommingBubble {
    if (!gIncommingBubble) {
        UIImage *defaultImage = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"ReceiverTextNodeBkg")];
        defaultImage = [defaultImage rtl_imageFlippedForRightToLeftLayoutDirection];
        UIEdgeInsets ei = UIEdgeInsetsFromString(@"{12,12,12,12}");
        ei = rtlEdgeInsetsWithInsets(ei);
...
+ (UIImage *)outgoingAnimatedHighlightedAlpha50 {
    if (!gOutgoingAnimatedHighlightedAlpha50) {
        UIImage *alpha50 = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath(@"SenderTextNodeBkg_alpha50")];
...
+ (UIImage *)outgoingAnimatedHighlightedAlpha20 {
    if (!gOutgoingAnimatedHighlightedAlpha20) {
        UIImage *alpha20 = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath(@"SenderTextNodeBkg_alpha20")];
...
+ (UIImage *)incommingAnimatedHighlightedAlpha50 {
    if (!gIncommingAnimatedHighlightedAlpha50) {
        UIImage *alpha50 = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath(@"ReceiverTextNodeBkg_alpha50")];
        alpha50 = [alpha50 rtl_imageFlippedForRightToLeftLayoutDirection];
        UIEdgeInsets ei = UIEdgeInsetsFromString(@"{12,12,12,12}");
        ei = rtlEdgeInsetsWithInsets(ei);
...
+ (UIImage *)incommingAnimatedHighlightedAlpha20 {
    if (!gIncommingAnimatedHighlightedAlpha20) {
        UIImage *alpha20 = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath(@"ReceiverTextNodeBkg_alpha20")];
        alpha20 = [alpha20 rtl_imageFlippedForRightToLeftLayoutDirection];
        UIEdgeInsets ei = UIEdgeInsetsFromString(@"{12,12,12,12}");
        ei = rtlEdgeInsetsWithInsets(ei);
