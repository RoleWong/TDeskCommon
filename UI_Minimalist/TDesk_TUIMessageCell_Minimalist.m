...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        for (int i = 1; i <= 45; ++i) {
            ...
            UIImage *image = [[TDeskImageCache sharedInstance] getResourceFromCache:imagePath];
...
- (void)prepareReactTagUI:(UIView *)containerView {
    ...
    [TDeskCore raiseExtension:TUICore_TUIChatExtension_ChatMessageReactPreview_MinimalistExtensionID parentView:containerView param:param];
}
...
- (void)updateConstraints {
    ...
    TDeskMessageCellLayout *cellLayout = self.messageData.cellLayout;
...
- (void)fillWithData:(TDeskMessageCellData *)data {
    ...
    if (data.showMessageModifyReplies) {
        ...
        for (NSDictionary *senderMap in self.messageData.messageModifyReplies) {
            ...
            TDeskRelationUserModel *userModel = self.messageData.additionalUserInfoResult[sender];
            ...
            if (avatarCount < avatarMaxCount - 1) {
                ...
                [avatarView setImage:[[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"msg_reply_more_icon")]];
            }
    ...
    _msgTimeLabel.text = [TDeskTool convertDateToHMStr:self.messageData.innerMessage.timestamp];
...
- (void)updateMessageStatus:(TUIMessageStatus)status {
    ...
    switch (status) {
        ...
        case TUIMessageStatus_Send_Succ: {
            _msgStatusView.image = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"msg_status_send_succ")];
        } break;
        case TUIMessageStatus_Some_People_Read: {
            _msgStatusView.image = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"msg_status_some_people_read")];
        } break;
        case TUIMessageStatus_All_People_Read: {
            _msgStatusView.image = [[TDeskImageCache sharedInstance] getResourceFromCache:TUIChatImagePath_Minimalist(@"msg_status_all_people_read")];
        } break;
...
+ (CGFloat)getHeight:(TDeskMessageCellData *)data withWidth:(CGFloat)width {
    NSAssert([data isKindOfClass:TDeskMessageCellData.class], @"data must be kind of TDeskMessageCellData");
